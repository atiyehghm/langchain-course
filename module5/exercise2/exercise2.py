...
def implement_execute_tool_call(
    tool_call: Dict, available_tools: List[Tool]
) -> Tuple[str, str]:
    ...
    while True:
        ...
        if implement_check_for_tool_calls(response=ai_message):
            ...
            for tool_call in getattr(ai_message, "tool_calls", []):
                tool_call_id, result = implement_execute_tool_call(
                    tool_call=tool_call, available_tools=available_tools
                )
